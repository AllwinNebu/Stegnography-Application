...
class SteganographyApp:
    ...
    def embed_data_in_image(self, image_path, data, output_path):
        ...
        if len(data) > max_bytes:
            raise ValueError(f"Data is too large. Image can only hide {max_bytes} bytes")
        
        # Embed length
        length_binary = format(len(data), '032b')
        idx = 0
        flat_array = image_array.flatten()
        
        # Embed length in more significant bits (MSB)
        for i in range(32):
            if idx >= len(flat_array):
                raise IndexError(f"Index {idx} exceeds flattened array size {len(flat_array)}")
            flat_array[idx] = (flat_array[idx] & 0x7F) | (int(length_binary[i]) << 7)  # Embed in MSB
            idx += 1
        
        # Embed data in more significant bits (MSB)
        for bit in binary_data:
            if idx >= len(flat_array):
                raise IndexError(f"Index {idx} exceeds flattened array size {len(flat_array)}")
            flat_array[idx] = (flat_array[idx] & 0x7F) | (int(bit) << 7)  # Embed in MSB
            idx += 1
