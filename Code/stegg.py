...
class SteganographyApp:
    ...
    def embed_data_in_image(self, image_array, binary_data, output_path):
        idx = 0
        flat_array = image_array.flatten()

        # Embed length of data (32 bits for the length)
        length_binary = format(len(binary_data), '032b')
        for i in range(32):
            flat_array[idx] = (flat_array[idx] & 0xFE) | (int(length_binary[i]) << 7)  # Embed in MSB
            idx += 1

        # Embed actual data (remaining bits)
        for bit in binary_data:
            if idx >= len(flat_array):
                raise ValueError("Index exceeds available space in the image array.")
    ...
    def hide_audio_in_image(self, image_path, audio_path, key, output_path):
        ...
        # First, determine how much space is required to store the data
        binary_data = ''.join(format(byte, '08b') for byte in encrypted_data)
        data_len = len(binary_data)

        # Prepare image and check if it has enough space
        image = Image.open(image_path)
        image_array = np.array(image)

        # Number of bits per pixel (3 channels per pixel)
        max_bits = image_array.size * 3  # For RGB image (no alpha channel)

        # If the image is not large enough, resize it
        if len(binary_data) > max_bits:
            # Calculate how many additional pixels are needed
            additional_bits_needed = len(binary_data) - max_bits
            additional_pixels_needed = additional_bits_needed // 3

            # Create a new image with additional pixels
            new_width = image_array.shape[1] + (additional_pixels_needed // image_array.shape[0])
            new_height = image_array.shape[0] + (additional_pixels_needed // image_array.shape[1])
            ...
            # Place the old image into the top-left corner of the new image
            new_image_array[:image_array.shape[0], :image_array.shape[1]] = image_array

            # Now new_image_array will have enough space for the data

            # Embed the data into the resized image
            self.embed_data_in_image(new_image_array, binary_data, output_path)
        else:
            # If the image has enough space, embed the data directly
            self.embed_data_in_image(image_array, binary_data, output_path)
    ...
    def hide_text_in_image(self, image_path, text, key, output_path):
        ...
        encrypted_data = self.encrypt_maes(data, key)
        binary_data = ''.join(format(byte, '08b') for byte in encrypted_data)
        self.hide_data_in_image(image_path, binary_data, output_path)
